...
+ (NSMutableArray*)binaryImages {
    ...
    for (uint32_t i = 0 ;i < imageCount; ++ i) {
        ...
        for(uint32_t iCmd = 0; iCmd < header->ncmds; iCmd++)  {
            ...
            switch(loadCmd->cmd) {
                case LC_SEGMENT: {
                    ...
                    if(strcmp(segCmd->segname, SEG_TEXT) == 0) { // 代码段
        ...
        NSString *imageInfo = [NSString stringWithFormat:@"0x%lx - 0x%lx %@ %@ <%@> %@",
                               (uintptr_t)header,
                               (uintptr_t)((uintptr_t)header + imageSize -1), // MachO 结束地址
                               imageName,
                               architecture, // 架构
                               uuidStr,
                               imageFullName];
