 ...
 * 监控原理：swizzling [NSObject dealloc]方法，dealloc时只调用析构，不调用free，同时把isa指针指向HYZombie，通过消息转发机制捕捉zombie对象
    * 存在问题:
        * 纯swift的class不是继承自NSObject, 不能通过这种方式hook⚠️
        * 虽然纯swift的前8个字节也是isa, 可以修改为Proxy野指针转发class, but如果method方法不是通过objc_msgSend方式调用, 而是虚函数表、直接函数地址调用的话, Proxy实现的`forwardInvocation`根本没用⚠️
 * 内存占用：延迟free和对象释放栈占用内存比较大，可以通过maxOccupyMemorySize设置最大内存，当收到memoryWarning或超出maxOccupyMemorySize后，通过FIFO机制释放对象，由于访问局部性，所以通过FIFO释放对象对捕捉效果影响不大
 ...
 DDZombieMonitor.sharedInstance().maxOccupyMemorySize = 50 * 1024 * 1024
 DDZombieMonitor.sharedInstance().start()
 DDZombieMonitor.sharedInstance().handle = { className, obj, selectorName, deallocStack, zombieStack in
     print("ZombieInfo = \"detect zombie class:%@ obj:%p sel:%@\ndealloc stack:%@\nzombie stack:%@\"", className ?? "", obj ?? 1, selectorName ?? "", deallocStack ?? "", zombieStack ?? "")
 }
 */
